...
def _normalize_hypotheses(h: Any) -> List[Dict[str, Any]]:
    ...
    for item in h:
        if isinstance(item, dict):
            ...
            try:
                # coerce via str() to avoid mypy complaining about Any | None
                conf = float(str(conf))
...
def reason(case: dict) -> UrgencyOutput:
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            if any(
                tok in msg
                for tok in ("503", "UNAVAILABLE", "overload", "overloaded", "rate")
            ):
                if attempt < max_attempts:
                    ...
                    raise RuntimeError(
                        f"Gemini service unavailable after {max_attempts} attempts: {e}"
                    )
    ...
    if resp is None:
        ...
        raw = _json_from_text(getattr(resp, "text", "{}") or "{}")
