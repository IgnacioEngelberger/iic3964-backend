...
def get_users_metrics(
    ...
    """
    Obtiene métricas de todos los usuarios (médicos residentes)
    """
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Error calculando métricas de usuarios"
        )
...
def get_user_metrics(
    ...
    """
    try:
        return metric_service.get_single_user_metrics(
            str(user_id), start_date, end_date
        )
    except Exception as e:
        print(f"Error fetching user metrics: {e}")
        raise HTTPException(
            status_code=500, detail="Error calculando métricas del usuario"
        )


@router.get(
    "/insurance_companies/{company_id}", response_model=MetricStats, tags=["Metrics"]
)
def get_insurance_metrics(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Error calculando métricas de aseguradora"
        )
