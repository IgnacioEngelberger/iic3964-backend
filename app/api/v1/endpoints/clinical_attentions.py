...
from app.schemas.clinical_attention import ClinicalAttentionsListResponse, ClinicalAttentionDetailResponse, CreateClinicalAttentionRequest, UpdateClinicalAttentionRequest, DeleteClinicalAttentionRequest
...
def get_clinical_attentions(
    ...
    search: str | None = Query(None, description="Buscar en paciente (RUT, nombre) o diagnóstico"),
    order: str | None = Query(None, description="Ordenamiento (ej. -created_at,diagnostic)")
):
    try:
...
def patch_clinical_attention(
    ...
    try:
        updated_attention = clinical_attention_service.update_attention(attention_id, payload)
        return updated_attention
    except HTTPException as e:
        raise e
    except Exception as e:
        print(f"Error en el endpoint (update): {e}")
        raise HTTPException(status_code=500, detail="Error interno al actualizar la atención clínica")
...
def delete_clinical_attention(attention_id: UUID, payload: DeleteClinicalAttentionRequest):
    try:
        ...
        raise HTTPException(status_code=500, detail="Ocurrió un error interno al eliminar")